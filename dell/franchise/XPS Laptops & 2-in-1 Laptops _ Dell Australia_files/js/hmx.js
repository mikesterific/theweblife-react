...
function isInfinityRTReady() {
  ...
  if (!(scene._projectparsed && scene._projectparsed)) {// ignore
...
function InfinityRTStart(gl, producturl) {
  ...
  scene.start();
  if (scene2) scene._nav = scene2._nav;
}
...
function InfinityRTStart2(gl, producturl) {
  ...
  scene2.start();
  if (scene) scene2._nav = scene._nav;
}
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  jsonhttp.onload = function (e) {
    ...
    var cmbs = [document.getElementById("cmbLeft"), document.getElementById("cmbRight")];
    ...
    for (j = 0; j < cmbs.length; j++) {
      for (var product in catalog.products) {
        ...
        opt.id = product;
        if (catalog.initial[j] == product) opt.selected = true;
    ...
    document.getElementById('leftButton').addEventListener('click', function () {
      ...
      if (leftDropdown) {
        document.getElementById("cmbLeft").style.display = 'block';
      } else {
    ...
    document.getElementById('rightButton').addEventListener('click', function () {
      ...
      if (rightDropdown) {
        document.getElementById("cmbRight").style.display = 'block';
      } else {
...
function frameUpdate() {
  if (scene._refineCount < 64 || scene2._refineCount < 64) {
    ...
    if (bGotoPosUpdate) updateBoth();
    if (initialloader && scene.loadProgressiveTextures && scene2.loadProgressiveTextures) hideInitialLoader();
  }
...
function mouseMove(ev) {
  ...
  if (!panNav) {
    mdelta[0] *= 0.2;
    mdelta[1] *= 0.2;
    if (s._nav.NavRotation(mpos, mdelta)) updateBoth();
  } else {
    var mdelta2 = [mdelta[0], mdelta[1]];
    if (s._nav.NavPan(mdelta2)) updateBoth();
  }
}
...
var animStoped = true;
var dragCursor;
var curBrowser = BrowserDetect.browser; // IE doesn't support co-ordinates
...
function handClosed() {
  dragCursor = curBrowser == "Firefox" ? "-moz-grabbing" : "url(images/closedhand.cur)" + cursCoords + ", move"; // Opera doesn't support url cursors and doesn't fall back well...
...
function touchMove(event) {
  ...
  var getClosest = function getClosest(touch, touches) {
  ...
  switch (event.touches.length) {
    case 1:
      if (doubleTouch == false) {
        touches[0].set(event.touches[0].pageX, event.touches[0].pageY, 0);
        touches[1].set(event.touches[0].pageX, event.touches[0].pageY, 0);
        if (s._nav.NavRotation([touches[0].x, touches[0].y], [(prevTouches[0].x - touches[0].x) * 0.5, (prevTouches[0].y - touches[0].y) * 0.5])) updateBoth();
      }
    ...
    case 2:
      ...
      if (s._nav.NavChangeDolly(deltaScene)) updateBoth();
...
//# sourceMappingURL=../maps/hmx.js.map
